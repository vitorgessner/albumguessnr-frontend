...
const ConfigComponent = ({ inSheet }: { inSheet?: boolean }) => {
    ...
    return (
        ...
        >
            <aside
                className={`flex flex-col left-0 ${!isGuessed ? '-mb-70' : '-mb-58'} pb-10 h-full min-w-dvw w-full lg:w-fit lg:min-w-fit lg:-ml-60 transition-all max-w-66 bg-(--card-light) border-t-2 border-border lg:bg-(--card-light) duration-400 ease-in-out translate-x-8 ${isExpanded && `lg:translate-x-60 lg:translate-y-0 ${!isGuessed ? '-translate-y-70 lg:-translate-y-60' : '-translate-y-50 lg:-translate-y-70'}`}`}
            >
